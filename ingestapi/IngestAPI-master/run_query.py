...
#constructs the query for mongodb - note inclusion is a list
def constructQuery(fields, inclusion, query_type, umls_db, populate_fields_by_synonym):
            ...
            if stripped == "AND":
                ...
                #INCLUDE ANOTHER ELIF HERE THAT CHECKS IF arg[0] IS ICD9 OR ICD10. IF SO LOWERCASE THE ARG[0]
                    ...
                    if "date" in arg[0]:
                        ...
                        #IF populate_fields_by_synonym IS TRUE THEN ADD THESE SYNONYMS TO THE FIELDS ALSO
                        ...
                        if len(synonyms) > 0:
                            ...
                            for term in synonyms:
                                ...
                                #for linkback
                                if populate_fields_by_synonym == "true":
                                    fields.append(synonyms[term] + ':' + term)
                ...
                if i > 0 and i < len(inclusion) -1 and (inclusion[i-1] != "AND" and inclusion[i-1] != "OR") and  (inclusion[i+1] != "AND" and inclusion[i+1] != "OR"):
                    query = query[:-1]
                    query = query + ']},'
                    booleans.pop()
                elif i == len(inclusion) -1 and len(booleans) > 0:  #close with extra set of brackets bc at the end of the expression - have to close outer boolean
                    query = query[:-1]
                    query = query + ']}'
                

    if len(inclusion[0]) > 0 and inclusion[0] != "all":
        if query[-1] == ",":
            query = query[:-1]
        query = query + ']},'
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 5:
        print "Run as python construct_query.py [fields] [inclusion] [query_type] [populate_fields_by_synonym]"
        sys.exit(-1)

    logger = logging.getLogger('myapp')
    hdlr = logging.FileHandler('/Users/gangopad/Company/REST/Logs/construct_query.log')
    ...
    populate_fields_by_synonym = sys.argv[4]
    new_inclusion = ""
    new_fields = ""


    logger.error("Inclusion: " + inclusion)
    ...
    if "_" in inclusion:
        ...
        for i in inclusion:
            if "id" in i:
                new_inclusion = new_inclusion + "_" + i
            else:
                new_inclusion = new_inclusion + " " + i

        inclusion = new_inclusion[1:]
        inclusion = inclusion.strip()
        inclusion = inclusion.replace(",", " AND ")

    #clean up fields (specifically for case of linkback)
    fields = fields.replace("_AND_", ",")
    ...
    query = constructQuery(fields, PN_expression, query_type, umls_db, populate_fields_by_synonym)
